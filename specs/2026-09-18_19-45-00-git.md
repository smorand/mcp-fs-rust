...
This document specifies the git capability: **14 MCP tools** (11 `git.*`, 3 `git.auth*`), a **blob-backed object database**, a **git smart HTTP server** at `/git/{mount_id}/` that a real `git` CLI can clone from and push to, and an **OAuth device flow** for GitHub and GitLab so a project can clone from a private remote.
...
`git/` is 4712 lines: `http/mod.rs` (1372), `oauth/device_flow.rs` (744), `odb.rs` (507), `db.rs` (464), `oauth/persistence.rs` (372), `oauth/store.rs` (361), `repo.rs` (357), `http/pktline.rs` (275), `oauth/cipher.rs` (209). The whole family is gated behind `git.enabled`.
...
- **Tools**: `tools/git.rs` (11 tools), `tools/git_auth.rs` (3 tools).
- **Object database**: `git/odb.rs`, key layout and the export/import pair (`:1-30`).
- **Index**: `git/db.rs`, three tables `git_objects`, `git_refs`, `git_remotes` (`:42`).
- **Repository store**: `git/repo.rs`, one entry per project, `state/git/{id}.db` plus `state/git-repos/{id}/` (`:1-13`).
- **Smart HTTP**: `git/http/mod.rs`, four routes (`:9-14`), pkt-line framing in `http/pktline.rs`.
- **OAuth**: `git/oauth/device_flow.rs` (RFC 8628), `store.rs`, `cipher.rs` (AES-256-GCM), `persistence.rs`.
...
- Git hooks, submodules, LFS, worktrees, sparse checkout, merge and rebase. None exists.
...
| `crates/mcp-fs/src/git/db.rs` | Specified, unchanged | FR-602 |
...
| `crates/mcp-fs/src/git/oauth/` | Specified, unchanged | FR-624..FR-629 |
| `crates/mcp-fs/src/tools/git.rs`, `git_auth.rs` | Specified, unchanged | FR-611..FR-614 |
