...
> Spec ID: SPEC-0001
> Source Specification: `specs/SPEC-0001_2026-09-23_08-47-57-mcp-tool-annotation-hints/spec.md`
> Nature: DEBT
> Depth: L
...
```
US-0001 (schema infra)
  ├── US-0002 (contract golden awareness)
  ├── US-0003 (admin/context7/db)      ─┐
  ├── US-0004 (doc/document/edit)       │
  ├── US-0005 (editor/git_auth/git_pr)  ├── all feed → US-0010 (registry-wide invariants)
  ├── US-0006 (listing/metadata/read)   │                  │
  ├── US-0007 (search/search_semantic/sqlite)              │
  ├── US-0008 (web/write/lifecycle)     │                  │
  └── US-0009 (git.rs)                 ─┘                  │
                                                            ▼
                                              US-0011 (golden regen + docs)
                                              [also depends on US-0002 for
...
- Requirements in spec (`DR-`): 8 total (DR-001..DR-008).
  - DR-006, DR-007 → US-0001 (exactly one story each).
  - DR-008 → US-0002 (infrastructure) and US-0011 (completion/exercise). This
    single requirement spans two stories by design: US-0002 prepares the
    mechanism, US-0011 is where the golden file is actually regenerated and the
  ...
  - DR-001, DR-002, DR-003, DR-004, DR-005 → these five rules are the
    ...
    tool-annotation stories (US-0003 through US-0009), because each rule spans
    all 123 tools and no single story's file-budget can hold the full set.
    Each of US-0003..US-0009 quotes the five rules verbatim and applies only the
...
- Tests in spec (`DT-`): 5 total (DT-001..DT-005).
  - DT-001, DT-002 → US-0001 (exactly one story each).
  - DT-003, DT-004 → US-0010 (exactly one story each).
  - DT-005 → prepared in US-0002 (the mechanism exists and passes vacuously),
    exercised to completion in US-0011 (the assertion that actually matters:
...
- SC-orphan FRs homed: DR-007 (behavior invariance) is a cross-cutting
  ...
  US-0001 (the story that first changes `to_list_entry()`, the highest-risk
...
- 123 per-tool classifications (Section 3.1): distributed exactly once each
  across US-0003 (17) + US-0004 (10) + US-0005 (13) + US-0006 (13) + US-0007 (16) +
  US-0008 (15) + US-0009 (39) = 123. Verified by direct source enumeration
...
- 24 touched files (Section 2): `mcp/schema.rs` (US-0001), `tools/contract_golden.rs`
  (US-0002), the 19 `tools/*.rs` family files (US-0003 through US-0009),
  `tools/all.rs` (US-0010), `tool-contract-golden.json` + `TOOL_CONTRACT.txt` +
  `.agent_docs/tools.md` (US-0011) = 1+1+19+1+3 = 25. Note: this is one more
